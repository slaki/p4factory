 ...
 *
 */
void bfm_set_pcap_outdir( char *outdir_name )
{
...
{
    return (port_num >= 1 && port_num <= bfm_port_count);
}
                   ...
                   int error_seen)
{
    ...
    for (idx = 0; idx < bfm_port_count; idx++) {
...
bfm_error_t
bfm_port_interface_add(char *ifname, uint32_t port_num,
		       char *sw_name, int dump_pcap)
{
    ...
    if(dump_pcap)
    {
        snprintf(vpi_spec, sizeof(vpi_spec),
                 "pcapdump|%s/bfns.%s-port%.2d.pcap", 
...
bfm_error_t
bfm_port_interface_remove(char *ifname)
{
    port_info_t *pi;
    int idx;
